[![GitHub License](https://img.shields.io/github/license/Vishu1197/docking-predictor)](LICENSE)
[![GitHub release](https://img.shields.io/github/v/release/Vishu1197/docking-predictor)](https://github.com/Vishu1197/docking-predictor/releases)
![Python](https://img.shields.io/badge/python-3.10%2B-blue)
![Status](https://img.shields.io/badge/status-Research--Grade-darkgreen)
![Platform](https://img.shields.io/badge/platform-CPU%20%7C%20CUDA-orange)
...
Prediction Accuracy) — Pearson ≈ 0.50 on the true hold-out
...
score; see the ensemble selection rule.
---
⚙️ Installation
```bash
...
`examples/sample_partial.csv` — 9 of the 466 possible
...
Training Pipeline) on a 17,500-row validation split held out
...
Full precision numbers: `outputs/leaderboard.csv`.
</details>
...
```
All defaults live in `configs/config.yaml`; every important
...
```
Stages in detail:
Data preprocessing. `build_pool()` reads every one of the 175 parquet chunks,
...
Feature engineering / preprocessing pipeline. `FeaturePipeline.fit()` computes,
...
Chunk processing. All 175 chunks are scanned; nothing is discarded up front
except non-physical target rows.
Outlier removal. See step 1 — target-side filtering, plus per-feature
winsorisation to bound extreme descriptor values (some raw cells reach ~3.5×10⁶)
before scaling.
Validation split. 10% validation, 10% internal test, 80% train (the internal
test split is separate from, and much smaller than, the true experimental hold-out
in `data/test/test.csv`).
Optuna hyperparameter optimisation. The four strongest gradient-boosting models
...
ML benchmark. All 12 classical algorithms are fit on the augmented training
pool (with row-capping for slow algorithms like SVR/KNN/RandomForest) and scored on
the validation split.
Deep-learning benchmark. Four architectures — `ResidualTabularNet`,
...
Automatic ensemble construction. From the six best-performing base models by
...
Automatic deployment of the best ensemble. The candidate with the lowest
...
Held-out test evaluation. Only after the manifest above is written does
...
---
💻 CLI Reference
Full reference: `docs/CLI.md`. Quick summary:
```bash
...
Prediction Accuracy). It should not replace molecular
...
chunks with a `score1` target column. See Training Pipeline.
...
---
🗒️ Changelog
See `CHANGELOG.md` and the v2.0.0 release notes
for the full history.
---
🙏 Acknowledgements
DockPred is built on top of scikit-learn,
LightGBM, XGBoost,
CatBoost, PyTorch, and
Optuna. Ligand descriptors follow the
PubChem compound-property schema.
---
🗂️ Project Structure
```
